...
struct PieceView: View {
  ...
  /// Whether the size popover is open.
  @State private var showsSizePopover = false

  /// The A of the score (rule 115): a button opening the sizes as a slider,
  /// small A to big A, with the annotations note living right there.
  private var sizeControl: some View {
    ...
    .buttonStyle(.plain)
    .foregroundStyle(.secondary)
    .help("Tamanho da partitura")
    .popover(isPresented: $showsSizePopover, arrowEdge: .top) {
      VStack(spacing: 12) {
        HStack(spacing: 12) {
          Text("A").font(.system(size: 12))
          Slider(value: sizeIndex, in: 0...Double(Self.sizeSteps.count - 1), step: 1)
            .frame(width: 220)
  ...
  /// The slider's position on the steps, writing straight to the size.
  private var sizeIndex: Binding<Double> {
    Binding(
      get: { Double(Self.sizeSteps.firstIndex(of: scoreUnits) ?? 2) },
      set: { value in
        let index = min(max(Int(value.rounded()), 0), Self.sizeSteps.count - 1)
        guard Self.sizeSteps[index] != scoreUnits else { return }
        scoreUnits = Self.sizeSteps[index]
      })
  }
